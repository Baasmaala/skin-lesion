"""
Train an EfficientNet-B3 classifier on the ISIC 2018 dermoscopic dataset —
an architecturally different model from the 5 ResNet50 fold models, to test
whether ensemble diversity helps with the class confusion the ResNet50
ensemble alone couldn't fix (MEL/NV and AKIEC/BKL — see
scripts/evaluate_kfold_ensemble.py's confusion matrix). Different
architectures tend to make different mistakes than same-architecture models
trained on different data slices.

First pass: trained on the ORIGINAL single train/val/test split (not
5-fold) — cheaper than training 5 more models. If this one model measurably
helps when combined with the ResNet50 ensemble (see
scripts/evaluate_diverse_ensemble.py), a full 5-fold EfficientNet run would
be the natural next step; if it doesn't help, we've learned that cheaply.

Uses EfficientNet's standard 300x300 input resolution (not the 224x224 the
ResNet50 models use) — each ensemble member can use its own preprocessing;
they only need to agree at the point where their softmax outputs are
averaged.

Requires:
    data/processed/splits.csv   (already exists from the original run)
    data/raw/ISIC2018_Task3_Training_Input/

Produces:
    results/checkpoints/efficientnet_b3_best.pt
    results/logs/efficientnet_b3_log.csv
...
from src.models.classifier import build_efficientnet_classifier  # noqa: E402
...
SEED = 42
IMG_SIZE = 300  # EfficientNet-B3's standard input resolution
BATCH_SIZE = 16
EPOCHS = 20
LR_BACKBONE = 1e-5
LR_HEAD = 1e-4
...
def main():
    ...
    if device.type != "cuda":
        print("WARNING: no GPU detected — unfrozen EfficientNet-B3 training will be extremely slow on CPU.")
    ...
    model = build_efficientnet_classifier(num_classes=7, freeze_backbone=False).to(device)

    # Two-LR optimizer: backbone moves gently, the new head moves faster.
    # EfficientNet's head is model.classifier.*, not model.fc.* like ResNet.
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(
                {"epoch": epoch, "model_state_dict": model.state_dict(), "val_macro_f1": val_f1, "img_size": IMG_SIZE},
                ckpt_path,
            )
